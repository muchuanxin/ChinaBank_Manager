...
public class HistogramView extends View{
    ...
    private void initPaint() {
       ...
       //histogramPaint.setColor(R.drawable.red_shape_gradient);
       // LinearGradient lg=new LinearGradient(0,0,0,600,Color.RED,Color.WHITE, Shader.TileMode.MIRROR);
        //参数一为渐变起初点坐标x位置，参数二为y轴位置，参数三和四分辨对应渐变终点，最后参数为平铺方式，这里设置为镜像Gradient是基于Shader类，所以我们通过Paint的setShader方法来设置这个渐变，代码如下:
       // histogramPaint.setShader(lg);
    }
       ...
       /* for (int i = yAxes.length-1; i>=0; i--) {
        ...
        if (data != null && data.length > 0) {
            for (int i = 0; i < data.length; i++) { // 循环遍历将柱状图画出来
                ...
                lg=new LinearGradient(rectf.left,rectf.top,rectf.left,rectf.bottom,getResources().getColor(R.color.red_histogram),Color.WHITE, Shader.TileMode.MIRROR);
                //参数一为渐变起初点坐标x位置，参数二为y轴位置，参数三和四分辨对应渐变终点，最后参数为平铺方式，这里设置为镜像Gradient是基于Shader类，所以我们通过Paint的setShader方法来设置这个渐变，代码如下:
                histogramPaint.setShader(lg);
